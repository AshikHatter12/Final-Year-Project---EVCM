...
const create = (req, res) => {
  ...
  form.parse(req, async (err, fields, files) => {
      ...
      if(files.attachment){
        let writestream2 = gridfs.openUploadStream(media._id + 'attachment', {
          contentType: files.attachment.type || 'binary/octet-stream'})
        fs.createReadStream(files.attachment.path).pipe(writestream2)
      }
      try {
...
const mediaByAttachmentId = async (req, res, next, id) => {
  try{
  let media = await Media.findById(id).populate('postedBy', '_id name').exec()
    if (!media)
      ...
      req.media = media
      let files = await gridfs.find({filename:media._id + 'attachment'}).toArray()
        if (!files[0]) {
...
const attachment = (req, res) => {
  ...
  if (range && typeof range === "string") {
    ...
    downloadStream.on('end', () => {
      ...
      res.header('Content-Length', req.file.length)
      res.header('Content-Type', req.file.contentType)
console.log(req.file._id + 'attachment')
console.log(req)
      let downloadStream = gridfs.openDownloadStream(req.file._id)
      downloadStream.pipe(res)
      downloadStream.on('error', () => {
